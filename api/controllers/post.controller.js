...
export const deletePost=async(req,res,next)=>{
   ...
   res.status(200).json('post has been deleted sccuessfully');
   }catch(err){
      next(err);
   }
}
...
export const updatePost = async(req,res,next)=>{
    if(req.params.isAdmin || req.user.id!==req.params.userId){
      return next(errorHandler(403,'You are not allowd to update this post'));
    }
